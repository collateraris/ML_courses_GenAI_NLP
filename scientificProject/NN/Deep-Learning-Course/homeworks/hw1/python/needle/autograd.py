...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        ...
        if node in node_to_output_grads_list:
            grad_sum = node_to_output_grads_list[node][0]
        ...
        if node.op is not None:
            ...
            node_grad = node.grad if hasattr(node, 'grad') and node.grad is not None else None
...
def topo_sort_dfs(node, visited, topo_order):
    ...
    # Add current node after all its inputs have been processed
